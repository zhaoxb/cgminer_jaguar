...
#define ICARUS_BUF_SIZE 9
// The size of a successful nonce read
#define ANT_READ_SIZE 8
#define ICARUS_READ_SIZE 9
#define ICARUS_READ_TEMP_SIZE 5

// Ensure the sizes are correct for the Serial read
#if (ICARUS_READ_SIZE != 9)
#error ICARUS_READ_SIZE must be 9
#endif
...
static void string_upper(unsigned char *s, size_t l)
{
	size_t i;
	for (i = 0; i < l; i++)
	{
			    ...
			    struct timeval *tv_finish, struct thr_info *thr, int read_time)
{
	...
	cgtime(tv_start);
	// change from read_ii to read_nl
	// add debug
	err = usb_read_ii_timeout(icarus, info->intinfo, (char *)buf,
					      info->nonce_size, &amt, read_time,
					      C_GETRESULTS);
#if 0
	err = usb_read_nl_timeout(icarus, (char *)buf,
					      info->nonce_size, &amt, read_time,
					      C_GETRESULTS);
#endif
	cgtime(tv_finish);
...
static struct cgpu_info *icarus_detect_one(struct libusb_device *dev, struct usb_find_devices *found)
{
	...
	unsigned char nonce_out[ICARUS_READ_SIZE+1];

	applog(LOG_DEBUG, "FUNCTION %s entry ****", __FUNCTION__);
	...
	// comment this off because of the raw data string is sent
...
cmr2_retry:
	...
	ok = false;
	while (!ok && tries-- > 0) {
		applog(LOG_DEBUG, "detect fpga timer %d", tries);
		...
		err = usb_write_ii(icarus, info->intinfo,
				   (char *)(&golden_ob), sizeof(golden_ob)-1, &amount, C_SENDWORK);

		if (err != LIBUSB_SUCCESS || amount != sizeof(golden_ob)-1)
			continue;

		memset(nonce_bin, 0, sizeof(nonce_bin));
		ret = icarus_get_nonce(icarus, nonce_bin, &tv_start, &tv_finish, NULL, 1000);

		// add debug
		memset(nonce_out, 0, sizeof(nonce_out));
		memcpy(nonce_out, nonce_bin, sizeof(nonce_bin));
		if (ret != ICA_NONCE_OK)
			continue;
		...
		if (strncmp(nonce_bin, golden_nonce, 8) == 0) {
...
static int64_t icarus_scanwork(struct thr_info *thr)
{
	...
	struct ICARUS_INFO *info = (struct ICARUS_INFO *)(icarus->device_data);
	int ret, err, amount;
	unsigned char nonce_bin[ICARUS_BUF_SIZE+1];
	struct ICARUS_WORK workdata;
	char *ob_hex;
	uint32_t nonce;
	...
	unsigned char send_work[ICARUS_WORK_SIZE*2 + ICARUS_MIDSTATE_SIZE*2 + 2];
	...
	memset((void *)(&send_work[0]), 0, sizeof(send_work));
	memcpy(&send_work[0], bin2hex(work->data + ICARUS_WORK_DATA_OFFSET, 12), ICARUS_WORK_SIZE*2);
	memcpy(&send_work[ICARUS_WORK_SIZE*2], bin2hex(work->midstate, 32), ICARUS_MIDSTATE_SIZE*2);
	string_upper(send_work, ICARUS_WORK_SIZE*2 + ICARUS_MIDSTATE_SIZE*2);
	strcat(send_work, "\n");
	...
	// change size to 32*2 + 12*2 bytes
	err = usb_write_ii(icarus, info->intinfo, (char *)(&send_work[0]), 
					ICARUS_WORK_SIZE*2 + ICARUS_MIDSTATE_SIZE*2 + 1, 
					&amount, C_SENDWORK);

	// add debug
	applog(LOG_DEBUG, "write work to fpga, return: %d, size of data: %d", err, amount);

	if (err < 0 || amount != sizeof(send_work)-1) {
	...
	if (opt_debug) {
		...
		applog(LOG_DEBUG, "%s%d: sent %s",
			icarus->drv->name, icarus->device_id, send_work);
		//free(ob_hex);
	}
	...
	// add debug
	applog(LOG_DEBUG, "[%s]: get nonce = %s, return value: %d", __FUNCTION__, nonce_bin, ret);
	...
	nonce = (uint32_t)strtol(nonce_bin, NULL, 16);
	applog(LOG_DEBUG, "nonce hex = 0x%8x", nonce);
	//nonce = htobe32(nonce);
